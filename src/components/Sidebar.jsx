...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar Aside */}
      <aside
        className={`fixed top-0 left-0 bottom-0 z-50 md:z-30 flex flex-col justify-between bg-[#0b0e1b] border-r border-[#1a223c] transition-all duration-300 select-none shadow-2xl md:shadow-none ${
          isOpenMobile
      ...
      >
        ...
        <div>
          <div className="h-16 flex items-center justify-between px-4 border-b border-[#1a223c]">
            ...
            <button
              onClick={onCloseMobile}
              className="md:hidden p-1.5 rounded-xl bg-white/5 text-slate-400 hover:text-white border border-white/10"
              title="Cerrar menú"
            >
              <X className="w-5 h-5" />
            </button>
